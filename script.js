...
document.addEventListener("DOMContentLoaded", function () {
  ...
  recognition.onresult = function (event) {
    const command = event.results[0][0].transcript.trim().toLowerCase(); // Get recognized speech
    const commandWords = command.split(" ");

    if (commandWords[0] === "add") {
      const taskName = commandWords.slice(1).join(" ");
      addTask(taskName);
  ...
  function deleteTask(taskName) {
    ...
    tasks.forEach(task => {
      const taskText = task.querySelector('p').textContent.trim().toLowerCase();
